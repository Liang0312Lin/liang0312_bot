...
async def on_reaction_add(reaction, user):
  ...
  if str(reaction.emoji) == "➕":
    ...
    for member in team_1:
        ...
        if team1.dm_channel is None:
            await reaction.message.channel.send(f"{team1.mention}，您已經關閉了私訊功能，請開啟私訊功能，謝謝。")
        else:
            await team1.send('你是隊伍一')
    for member in team_2:
        team_2_id = f"{data[member]['uid']}\n"
        team2 = await client.fetch_user(team_2_id)
        if team2.dm_channel is None:
            await reaction.message.channel.send(f"{team2.mention}，您已經關閉了私訊功能，請開啟私訊功能，謝謝。")
        else:
